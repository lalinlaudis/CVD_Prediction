...
def train_model():
    ...
    try:
        ...
        for column, categories in categorical_features.items():
            ...
            encoder.fit(categories)  # Train encoder on all possible categories
...
def reset_fields():
    ...
    confusion_matrix_canvas.config(image="")
    roc_curve_canvas.config(image="")
        ...
        for feature in selected_features:
            if feature in encoders:
                ...
                if value not in encoders[feature].classes_:
                    value = encoders[feature].classes_[0]  # Replace with default
...
# Add field for Patient Name
name_frame = tk.Frame(left_frame)
name_frame.pack(pady=5, anchor="w")
tk.Label(name_frame, text="Patient Name:", width=30, anchor="w").pack(side="left")
entries["Patient Name"] = tk.Entry(name_frame, justify="left", width=20)
entries["Patient Name"].pack(side="left")
...
tk.Button(left_frame, text="Reset Fields", command=reset_fields, bg="red", fg="white").pack(pady=10)
